...
Bursh.prototype.drawing = function() {
  noStroke();
  // inspire by Jason Labbe https://openprocessing.org/sketch/400658
  if (this.shapeType === "CIRCLE") { 
    ...
    //different size circle will swell or shrink at the same time.
    //totation included
    ...
    if(isBlackHOle){ //triangle movement under the blackhole mode
      this.position.x+=(mouseX-this.position.x)/200;
      this.position.y+=(mouseY-this.position.y)/200;
    }
    else if(isreleased){ //triangle movement under the release mode
      this.position.x+=this.rleasedX/10;
      this.position.y+=this.rleasedY/10;
    }

    if(this.isControled){ //triangle movement under the isControled mode
      this.position.x+=(mouseX-this.position.x)/10;
      this.position.y+=(mouseY-this.position.y)/10;
    }
    //movement of TRIANGLE
    // triangle will changing the facing dircetion due to camra according to time past  and totate as whole
    ...
    // one base line and one effect line
    //fading effect     inspire from the project 1 "the breathing effect on titles "
    ...
    if(isBlackHOle){ //cube movement under the blackhole mode
      this.position.x+=(mouseX-this.position.x)/200;
      this.position.y+=(mouseY-this.position.y)/200;
    }
    else if(isreleased){ //cube movement under the blackhole mode
      this.position.x+=this.rleasedX/10;
      this.position.y+=this.rleasedY/10;
    }

    if(this.isControled){//cube movement under the blackhole mode
      this.position.x+=(mouseX-this.position.x)/10;
      this.position.y+=(mouseY-this.position.y)/10;
    }
    //movement of CUBE
    //cube will group swell or shrink.  and individual totate
    translate(this.position.x, this.position.y);
    rotate(this.rotateAngle);
    fill(this.size.x * this.R / 10, this.size.x * this.G / 10, this.size.x * this.B / 10, round(sin(this.timepast) * 128));
    ...
    resetMatrix();
  //movement of ballx
  //Randomly give the resulting circle a speed on the x axis and a speed on the y axis,
  // and reverse its speed when it hits a wall
    ...
    resetMatrix();
    //movement of snow
    //Generate snowflakes of random size, and give them a gravitational acceleration G to make them fall.
    // the wind effect is made for the snow and sakura
  ...
  //get the idea of the peasona 5 art book
  //A bit of an infinite loop, achieved by the periodic resetting of the size of the inner pentacle
    ...
    resetMatrix();
    //movement of sakura
    // sakura share same principle as snowflakes, replace snowflakes with petals
...
//add the update function to update all the brushes
